...
#include <string>           // building file paths at runtime
#include <vector>           // small list of folders to search for the assets
#include <filesystem>       // checking which folder actually has the assets
...
// I kept tripping over the "Impossible to open shader" error whenever I ran the
// .exe outside of Visual Studio, so this helper figures out where the assets
// really live instead of assuming the working directory is correct. It checks
// the current folder first, then walks up a couple of levels (handy for the
// x64\Debug build output), and falls back to the exe's own directory.
std::string FindAssetRoot()
{
	namespace fs = std::filesystem;

	// the relative paths we use later are all built off of "Utilities", so I just
	// look for that folder to know I'm in the right spot
	const std::string marker = "Utilities/shaders/vertexShader.glsl";

	// candidate roots to try, in order of how likely they are to be correct
	std::vector<fs::path> candidates;
	candidates.push_back(fs::current_path());                 // VS F5 sets this to ProjectDir
	candidates.push_back(fs::current_path().parent_path());   // one level up
	candidates.push_back(fs::current_path().parent_path().parent_path()); // x64\Debug -> project
	...
	// nothing matched, so just return the current directory and let the original
	// error message show up like before
...
int main(int argc, char* argv[])
{
	...
	// figure out where the assets actually live and switch into that folder so
	// every relative path after this (shaders AND textures) resolves correctly
	// no matter how the program got launched (VS F5, double-click, or terminal)
	std::string assetRoot = FindAssetRoot();
